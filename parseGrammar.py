...
#include "parserDef.h"\n""")
                ...
                for rhs_elem in rhs:
                    ...
                    if is_terminal:
                        ...
                        if rhs_elem in seen_symbols:
                            continue
                        f.write("struct ruleRightSideElement " + rhs_elem.lower() + " = {")
                        ...
                        if rhs_elem in seen_symbols:
                            continue
                        f.write("struct ruleRightSideElement " + rhs_elem[1:-1] + " = {")
                        f.write(str(is_epsilon).lower() + ", " + str(is_terminal).lower() + ", ")
                        f.write(".symbol.nonTerminal = NT_" + rhs_elem[1].capitalize() + rhs_elem[2:-1])
                        f.write("};\n")
                    seen_symbols.add(rhs_elem)

                f.write ("RuleRightSideElement rhs" + str(rule_num) + "[] = {")
                for rhs_elem in rule_rhs:
                    f.write(f"&{rhs_elem}, ")
                f.write("};\n")
                f.write("struct rule rule" + str(rule_num) + " = {")
        ...
        for i in range(rule_num):
            f.write("&rule" + str(i) + ", ")
        f.write("};\n")
        f.write("struct grammar languageGrammar = {NT_Program, " + str(rule_num) + ", rules};\n")
