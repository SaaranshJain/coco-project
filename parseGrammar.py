...
#include "parserDef.h"\n""")
        ...
        for line in lines:
            ...
            for rhs in rhs_arr:
                ...
                if not iseps:
                    ...
                    for rhs_elem in rhs:
                        ...
                        if is_terminal:
                            ...
                            f.write(str(is_terminal).lower() + ", ")
                            f.write(f".symbol.terminal = {rhs_elem}")
                            f.write("};\n")
                            ...
                            f.write(str(is_terminal).lower() + ", ")
                            f.write(".symbol.nonTerminal = NT_" + rhs_elem[1:-1].upper())
                            f.write("};\n")
                ...
                f.write(".lhs = NT_" + lhs.strip()[1:-1].upper() + ", ")
                if not iseps:
                    f.write(".rhs = _rhs" + str(rule_num) + ", ")
                    f.write(".isEpsilon = false, ")
                    f.write(".rhsLength = " + str(len(rule_rhs)) + "};\n")
                else:
                    f.write(".isEpsilon = true};\n")
                rule_num += 1
        f.write("\nRule _grammarRules[] = {")
